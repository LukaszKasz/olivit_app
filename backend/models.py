from sqlalchemy import Column, Integer, String, DateTime, Boolean, UniqueConstraint
from sqlalchemy.sql import func
from database import Base


class User(Base):
    """
    User model for authentication.
    Stores user credentials and basic information.
    """
    __tablename__ = "users"

    id = Column(Integer, primary_key=True, index=True)
    username = Column(String(50), unique=True, index=True, nullable=False)
    email = Column(String(100), unique=True, index=True, nullable=False)
    hashed_password = Column(String(255), nullable=False)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    
    def __repr__(self):
        return f"<User(username={self.username}, email={self.email})>"


class IntegrationSettings(Base):
    """
    Integration credentials/settings stored in DB.
    One row per provider: prestashop / woocommerce / baselinker.
    """
    __tablename__ = "integration_settings"

    id = Column(Integer, primary_key=True, index=True)
    provider = Column(String(50), unique=True, index=True, nullable=False)
    base_url = Column(String(255), nullable=False)
    api_key = Column(String(255), nullable=True)
    consumer_key = Column(String(255), nullable=True)
    consumer_secret = Column(String(255), nullable=True)
    access_token_secret = Column(String(255), nullable=True)
    verify_ssl = Column(Boolean, nullable=False, default=True)
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())


class MainProduct(Base):
    __tablename__ = "main_products"

    id = Column(Integer, primary_key=True, index=True)
    project_number = Column(String(50), index=True, nullable=False)
    name = Column(String(255), nullable=False)
    id_szczegolow_produktu = Column(Integer, nullable=True, index=True)
    order_index = Column(Integer, nullable=False, index=True)


class MainProductTestOrder(Base):
    __tablename__ = "main_product_test_orders"

    id = Column(Integer, primary_key=True, index=True)
    project_number = Column(String(50), index=True, nullable=False)
    name = Column(String(255), nullable=False)
    laboratory_name = Column(String(100), index=True, nullable=True)
    batch_number = Column(String(255), nullable=True)
    asana_task_number = Column(String(255), nullable=True)
    test_cost = Column(String(255), nullable=True)
    production_date = Column(String(50), nullable=True)
    expiry_date = Column(String(50), nullable=True)
    planned_test_date = Column(String(50), nullable=True)
    test_cost = Column(String(255), nullable=True)
    po_number = Column(String(255), nullable=True)
    workflow_status = Column(String(50), nullable=False, server_default="ordered_tests", index=True)
    clarification_note = Column(String(2000), nullable=True)
    ordered_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False, index=True)


class VariantProduct(Base):
    __tablename__ = "variant_products"

    id = Column(Integer, primary_key=True, index=True)
    sku = Column(String(100), index=True, nullable=False)
    name = Column(String(512), nullable=False)
    ean = Column(String(255), nullable=False)
    order_index = Column(Integer, nullable=False, index=True)


class VariantProductBatchTestOrder(Base):
    __tablename__ = "variant_product_batch_test_orders"

    id = Column(Integer, primary_key=True, index=True)
    original_test_order_id = Column(Integer, index=True, nullable=True)
    sku = Column(String(100), index=True, nullable=False)
    name = Column(String(512), nullable=False)
    ean = Column(String(255), nullable=False)
    laboratory_name = Column(String(100), index=True, nullable=True)
    batch_number = Column(String(255), nullable=False)
    asana_task_number = Column(String(255), nullable=True)
    purchase_order_number = Column(String(255), nullable=True)
    test_cost = Column(String(255), nullable=True)
    production_date = Column(String(50), nullable=True)
    expiry_date = Column(String(50), nullable=True)
    planned_test_date = Column(String(50), nullable=True)
    test_cost = Column(String(255), nullable=True)
    po_number = Column(String(255), nullable=True)
    workflow_status = Column(String(50), nullable=False, server_default="ordered_tests", index=True)
    clarification_note = Column(String(2000), nullable=True)
    label_status = Column(String(50), nullable=True, index=True)
    batch_added_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False, index=True)
    ordered_at = Column(DateTime(timezone=True), nullable=True, index=True)
    printed_material_type = Column(String(100), nullable=True)
    product_name = Column(String(255), nullable=True)
    product_project_number = Column(String(100), nullable=True)
    product_ean_number = Column(String(255), nullable=True)
    product_batch_number = Column(String(255), nullable=True)
    product_expiry_date = Column(String(50), nullable=True)
    control_date = Column(String(50), nullable=True)
    market_label_version = Column(String(255), nullable=True)
    active_substances_match_pds = Column(String(50), nullable=True)
    active_substances_match_pds_note = Column(String(2000), nullable=True)
    label_version_matches_used_version = Column(String(10), nullable=True)
    label_version_matches_used_version_note = Column(String(2000), nullable=True)
    has_printing_errors = Column(String(10), nullable=True)
    has_printing_errors_note = Column(String(2000), nullable=True)
    has_graphic_design_errors = Column(String(10), nullable=True)
    has_graphic_design_errors_note = Column(String(2000), nullable=True)
    print_correctness = Column(String(10), nullable=True)
    print_correctness_note = Column(String(2000), nullable=True)
    has_labeling_errors = Column(String(10), nullable=True)
    has_labeling_errors_note = Column(String(2000), nullable=True)
    cap_is_correct = Column(String(20), nullable=True)
    cap_is_correct_note = Column(String(2000), nullable=True)
    induction_seal_weld_correct = Column(String(20), nullable=True)
    induction_seal_weld_correct_note = Column(String(2000), nullable=True)
    induction_seal_opening_correct = Column(String(20), nullable=True)
    induction_seal_opening_correct_note = Column(String(2000), nullable=True)
    package_is_dirty = Column(String(10), nullable=True)
    package_is_dirty_note = Column(String(2000), nullable=True)
    package_is_damaged = Column(String(10), nullable=True)
    package_is_damaged_note = Column(String(2000), nullable=True)
    qr_code_is_active = Column(String(20), nullable=True)
    qr_code_is_active_note = Column(String(2000), nullable=True)
    package_contents_match_card = Column(String(10), nullable=True)
    package_contents_match_card_note = Column(String(2000), nullable=True)
    product_verified = Column(String(10), nullable=True)
    product_verified_note = Column(String(2000), nullable=True)
    comment = Column(String(2000), nullable=True)
    linked_document_names = Column(String(4000), nullable=True)
    control_saved_at = Column(DateTime(timezone=True), nullable=True, index=True)


class VariantProductBatchTestOrderArchive(Base):
    __tablename__ = "variant_product_batch_test_orders_archive"

    id = Column(Integer, primary_key=True, index=True)
    ordered_test_id = Column(Integer, index=True, nullable=True)
    original_test_order_id = Column(Integer, index=True, nullable=True)
    sku = Column(String(100), index=True, nullable=False)
    name = Column(String(512), nullable=False)
    ean = Column(String(255), nullable=False)
    laboratory_name = Column(String(100), index=True, nullable=True)
    batch_number = Column(String(255), nullable=False)
    asana_task_number = Column(String(255), nullable=True)
    purchase_order_number = Column(String(255), nullable=True)
    test_cost = Column(String(255), nullable=True)
    production_date = Column(String(50), nullable=True)
    expiry_date = Column(String(50), nullable=True)
    planned_test_date = Column(String(50), nullable=True)
    test_cost = Column(String(255), nullable=True)
    po_number = Column(String(255), nullable=True)
    workflow_status = Column(String(50), nullable=True, index=True)
    clarification_note = Column(String(2000), nullable=True)
    label_status = Column(String(50), nullable=True, index=True)
    batch_added_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False, index=True)
    ordered_at = Column(DateTime(timezone=True), nullable=True, index=True)
    printed_material_type = Column(String(100), nullable=True)
    product_name = Column(String(255), nullable=True)
    product_project_number = Column(String(100), nullable=True)
    product_ean_number = Column(String(255), nullable=True)
    product_batch_number = Column(String(255), nullable=True)
    product_expiry_date = Column(String(50), nullable=True)
    control_date = Column(String(50), nullable=True)
    market_label_version = Column(String(255), nullable=True)
    active_substances_match_pds = Column(String(50), nullable=True)
    active_substances_match_pds_note = Column(String(2000), nullable=True)
    label_version_matches_used_version = Column(String(10), nullable=True)
    label_version_matches_used_version_note = Column(String(2000), nullable=True)
    has_printing_errors = Column(String(10), nullable=True)
    has_printing_errors_note = Column(String(2000), nullable=True)
    has_graphic_design_errors = Column(String(10), nullable=True)
    has_graphic_design_errors_note = Column(String(2000), nullable=True)
    print_correctness = Column(String(10), nullable=True)
    print_correctness_note = Column(String(2000), nullable=True)
    has_labeling_errors = Column(String(10), nullable=True)
    has_labeling_errors_note = Column(String(2000), nullable=True)
    cap_is_correct = Column(String(20), nullable=True)
    cap_is_correct_note = Column(String(2000), nullable=True)
    induction_seal_weld_correct = Column(String(20), nullable=True)
    induction_seal_weld_correct_note = Column(String(2000), nullable=True)
    induction_seal_opening_correct = Column(String(20), nullable=True)
    induction_seal_opening_correct_note = Column(String(2000), nullable=True)
    package_is_dirty = Column(String(10), nullable=True)
    package_is_dirty_note = Column(String(2000), nullable=True)
    package_is_damaged = Column(String(10), nullable=True)
    package_is_damaged_note = Column(String(2000), nullable=True)
    qr_code_is_active = Column(String(20), nullable=True)
    qr_code_is_active_note = Column(String(2000), nullable=True)
    package_contents_match_card = Column(String(10), nullable=True)
    package_contents_match_card_note = Column(String(2000), nullable=True)
    product_verified = Column(String(10), nullable=True)
    product_verified_note = Column(String(2000), nullable=True)
    comment = Column(String(2000), nullable=True)
    linked_document_names = Column(String(4000), nullable=True)
    control_saved_at = Column(DateTime(timezone=True), nullable=True, index=True)
    archived_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False, index=True)


class VariantProductBatchLabResult(Base):
    __tablename__ = "variant_product_batch_lab_results"
    __table_args__ = (
        UniqueConstraint("ordered_test_id", "detail_id", name="uq_variant_batch_lab_result_order_detail"),
    )

    id = Column(Integer, primary_key=True, index=True)
    ordered_test_id = Column(Integer, nullable=False, index=True)
    detail_id = Column(Integer, nullable=False, index=True)
    result_value = Column(String(2000), nullable=False)
    notes = Column(String(2000), nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False, index=True)


class VariantProductFinishedProductControl(Base):
    __tablename__ = "variant_product_finished_product_controls"

    id = Column(Integer, primary_key=True, index=True)
    ordered_test_id = Column(Integer, index=True, nullable=True)
    sku = Column(String(100), index=True, nullable=False)
    name = Column(String(512), nullable=False)
    ean = Column(String(255), nullable=False)
    laboratory_name = Column(String(100), index=True, nullable=True)
    asana_task_number = Column(String(255), nullable=True)
    label_status = Column(String(50), nullable=False, server_default="current", index=True)
    printed_material_type = Column(String(100), nullable=False)
    product_name = Column(String(255), nullable=False)
    product_project_number = Column(String(100), nullable=False)
    product_ean_number = Column(String(255), nullable=False)
    product_batch_number = Column(String(255), nullable=False)
    sample_location = Column(String(255), nullable=True)
    product_expiry_date = Column(String(50), nullable=False)
    control_date = Column(String(50), nullable=False)
    market_label_version = Column(String(255), nullable=False)
    active_substances_match_pds = Column(String(50), nullable=False)
    active_substances_match_pds_note = Column(String(2000), nullable=True)
    label_version_matches_used_version = Column(String(10), nullable=False)
    label_version_matches_used_version_note = Column(String(2000), nullable=True)
    has_printing_errors = Column(String(10), nullable=False)
    has_printing_errors_note = Column(String(2000), nullable=True)
    has_graphic_design_errors = Column(String(10), nullable=False)
    has_graphic_design_errors_note = Column(String(2000), nullable=True)
    print_correctness = Column(String(10), nullable=False)
    print_correctness_note = Column(String(2000), nullable=True)
    has_labeling_errors = Column(String(10), nullable=False)
    has_labeling_errors_note = Column(String(2000), nullable=True)
    cap_is_correct = Column(String(20), nullable=False)
    cap_is_correct_note = Column(String(2000), nullable=True)
    induction_seal_weld_correct = Column(String(20), nullable=False)
    induction_seal_weld_correct_note = Column(String(2000), nullable=True)
    induction_seal_opening_correct = Column(String(20), nullable=False)
    induction_seal_opening_correct_note = Column(String(2000), nullable=True)
    package_is_dirty = Column(String(10), nullable=False)
    package_is_dirty_note = Column(String(2000), nullable=True)
    package_is_damaged = Column(String(10), nullable=False)
    package_is_damaged_note = Column(String(2000), nullable=True)
    qr_code_is_active = Column(String(20), nullable=False)
    qr_code_is_active_note = Column(String(2000), nullable=True)
    package_contents_match_card = Column(String(10), nullable=False)
    package_contents_match_card_note = Column(String(2000), nullable=True)
    product_verified = Column(String(10), nullable=False)
    product_verified_note = Column(String(2000), nullable=True)
    carton_market_label_version = Column(String(255), nullable=True)
    carton_active_substances_match_pds = Column(String(50), nullable=True)
    carton_active_substances_match_pds_note = Column(String(2000), nullable=True)
    carton_label_version_matches_used_version = Column(String(10), nullable=True)
    carton_label_version_matches_used_version_note = Column(String(2000), nullable=True)
    carton_has_printing_errors = Column(String(10), nullable=True)
    carton_has_printing_errors_note = Column(String(2000), nullable=True)
    carton_has_graphic_design_errors = Column(String(10), nullable=True)
    carton_has_graphic_design_errors_note = Column(String(2000), nullable=True)
    carton_print_correctness = Column(String(10), nullable=True)
    carton_print_correctness_note = Column(String(2000), nullable=True)
    carton_has_labeling_errors = Column(String(10), nullable=True)
    carton_has_labeling_errors_note = Column(String(2000), nullable=True)
    carton_cap_is_correct = Column(String(20), nullable=True)
    carton_cap_is_correct_note = Column(String(2000), nullable=True)
    carton_induction_seal_weld_correct = Column(String(20), nullable=True)
    carton_induction_seal_weld_correct_note = Column(String(2000), nullable=True)
    carton_induction_seal_opening_correct = Column(String(20), nullable=True)
    carton_induction_seal_opening_correct_note = Column(String(2000), nullable=True)
    carton_package_is_dirty = Column(String(10), nullable=True)
    carton_package_is_dirty_note = Column(String(2000), nullable=True)
    carton_package_is_damaged = Column(String(10), nullable=True)
    carton_package_is_damaged_note = Column(String(2000), nullable=True)
    carton_qr_code_is_active = Column(String(20), nullable=True)
    carton_qr_code_is_active_note = Column(String(2000), nullable=True)
    carton_package_contents_match_card = Column(String(10), nullable=True)
    carton_package_contents_match_card_note = Column(String(2000), nullable=True)
    carton_product_verified = Column(String(10), nullable=True)
    carton_product_verified_note = Column(String(2000), nullable=True)
    comment = Column(String(2000), nullable=True)
    linked_document_names = Column(String(4000), nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False, index=True)


class ProductDetailedParameter(Base):
    __tablename__ = "product_detailed_parameters"

    id = Column(Integer, primary_key=True, index=True)
    id_szczegolow_produktu = Column(Integer, nullable=True, index=True)
    parameter_type_pl = Column(String(100), nullable=False)
    parameter_type_en = Column(String(100), nullable=False)
    parameter_name_pl = Column(String(255), nullable=False)
    parameter_name_en = Column(String(255), nullable=False)
    requirement_pl = Column(String(1000), nullable=False)
    requirement_en = Column(String(1000), nullable=False)
    method_pl = Column(String(1000), nullable=False)
    method_en = Column(String(1000), nullable=False)
    confirmation_pl = Column(String(1000), nullable=True)
    confirmation_en = Column(String(1000), nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False, index=True)
